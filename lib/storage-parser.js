...
class StorageParser {
    parseStorage(helpers, input, parentOutput, storage, options) {
        ...
        const isDataSource = helpers.isDataSource(reference);
        const { object } = construct;
        let { path } = construct;

        if (isFlowInput) {
            let flowInput = options.flowInputMetadata;
            if (flowInput?.metadata) {
                flowInput = {
                    ...options.flowInputMetadata,
                    metadata: { [reference]: flowInput.metadata[reference] }
                };
            }
            parentOutput.push({
                type,
            ...
            path = undefined;
            let metadata = options.dataSourceMetadata;
            if (metadata) {
                metadata = metadata[reference];
            }
            parentOutput.push({
                type,
                node: INPUTS.DATASOURCE,
                result: metadata
            });
        }
        ...
        if (isWaitBatch) {
            ...
            const parentStorageResult = Array.isArray(parentResult)
                ? parentResult.map(r => this._createResult({ ...r, path }))
                : this._createResult({ ...parentResult, path });

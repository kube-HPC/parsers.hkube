    ...
    _extractValueFromInput(options, input, settings, info) {
        ...
        if (isBatchRaw) {
            ...
            if (isFlowInput) {
                ...
                const res = parent.result;
                if ((type === RELATIONS.WAIT_BATCH || type === RELATIONS.WAIT_ANY_BATCH) && (Array.isArray(res))) {
                    result = [];
                    res.forEach(r => {
                        const uuid = uuidv4();
                        info.storage[uuid] = { accessor: r, path: construct.path };
                        result.push(`${INPUTS.STORAGE}${uuid}`);
                    })
                }
                else {
                    const uuid = uuidv4();
                    info.storage[uuid] = { accessor: res, path: construct.path };
                    result = `${INPUTS.STORAGE}${uuid}`;
                }
            }
        }
        return result;

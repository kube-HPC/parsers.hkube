...
class InputParser {
    ...
    objectToMetadata(object, paths) {
        paths = paths || [];
        const metadata = {};
        const keys = Object.keys(flatten(object));
        paths.forEach((p) => {
            let path = keys.includes(p);
            if (!path) {
                path = keys.find(k => k.startsWith(p));
            }
            if ((path) && !(p in metadata)) {
                const value = objectPath.get(object, p);

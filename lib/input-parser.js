...
class InputParser {
    ...
    cartesianx(a, b) {
        return [].concat(...a.input.map(d => b.input.map(e => [].concat(d, e))));
    }

    cartesian(a, b, ...c) {
        return (b ? this.cartesian(this.cartesianx(a, b), ...c) : a);
    }
    ...
    _parseBatchInput(options, storageData) {
        ...
        nodeInput.forEach((inp, index) => {
            const path = [];
            const data = this._createBatch(options, inp, path, storageData);
            if (Array.isArray(data)) {
                isBatch = true;
                if (data.length > maxLength) {
                    maxLength = data.length;
                }
                batchIndices.push({ index, data, path, storageData });
            }
        });
        /* eslint-disable */
        for (let index = 0; index < maxLength; index++) {
            ...
            batch.push({ input, storage });
        }
        if (operation === 'matrix') {
            batch = this.cartesian(...batch);
        }

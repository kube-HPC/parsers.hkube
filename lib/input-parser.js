...
class InputParser {
        ...
        if (isWaitBatch) {
            ...
            if (Array.isArray(parentResult)) {
                parentResult.filter(r => r).forEach((r) => {
                    const key = this._createStorage(storage, { ...r, path: construct.path });
                    result.push(key);
                });
            }
            else {
                const metadata = this._findMetadata(parentResult.metadata, reference);
                result = this._createStorageArray(metadata, storage, parentResult, construct.path);
            }
        }
        else {
            const parentResultInfo = Array.isArray(parentResult) ? parentResult.map(r => r) : parentResult;
            const key = this._createStorage(storage, { ...parentResultInfo, path: construct.path });
            result = key;
        }
        return result;
    }
    ...
    _createStorageArray(metadata, storage, parentResult, path) {
        ...
        if (Number.isInteger(metadata.size) && metadata.type === 'array') {
            Array.from(Array(metadata.size).keys()).forEach((i) => {
                const key = this._createStorage(storage, { ...parentResult, path, index: i });
                result.push(key);
            });
        }
        return result;
    ...
    _createResult(data) {
        ...
        const result = {
            ...data.storageInfo && { storageInfo: data.storageInfo },
    ...
    _createStorage(storage, data) {
        const uuid = uuidv4();
        storage[uuid] = this._createResult(data);

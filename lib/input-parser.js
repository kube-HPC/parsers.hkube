...
class InputParser {
    ...
    replaceFlowInput(pipeline) {
        ...
        if (pipeline.flowInput) {
            const flatInput = Object.values(flatten(pipeline.nodeInput)).map(v => this._extractObjectFromInput(v));
            flatInput.forEach((path) => {
                if (typeof path === 'string' && path.includes(INPUTS.FLOW_INPUT)) {
                    const value = objectPath.get(pipeline, path);
                    Array.isArray(value) ?
                        metadata[path] = { type: 'array', size: value.length } :
                        metadata[path] = { type: typeof (value) };
                }
            });
        }
        return metadata;
    ...
    _extractValueFromInput(options, input, settings, info) {
        ...
        if (isBatchRaw) {
            ...
            if ((settings.parseBatch || settings.parseParentNode) && (parentOutput)) {

...
class InputParser {
    ...
    _parseBatchInput(options, storageData) {
        ...
        nodeInput.forEach((inp, index) => {
            ...
            const path = [];
            let data;
            const batchResult = this._createBatch(options, inp, path, storageData);
            ...
            batchIndices.push({ index, data, path, storageData, batch });
        });

        if (operation === 'matrix') {
            const batchData = batchIndices.filter(b => b.batch).map(b => b.data);
            const noBatch = batchIndices.filter(b => !b.batch);
            const product = this.product(...batchData);
            product.forEach((p) => {
                const input = [];
                for (let i = 0; i < nodeInput.length; i++) {
                    const found = noBatch.find(n => n.index === i);
                    input[i] = (found && found.data) || p.shift();
                    // this.setInput(a.path, a.index, p, a.data);
                }
            ...
            for (let index = 0; index < maxLength; index++) {
                ...
                batchIndices.forEach((b) => {
                    const { cleanStorage, isStorage } = this._filterStorage(b.data, storageData);
                    storage = clone(cleanStorage);

                    const data = b.batch ? b.data[index] : b.data;
                    this.setInput(b.path, b.index, input, data);

                    if (isStorage) {
                        const storageKey = data.substr(2);
    ...
    setInput(path, index, input, data) {
        if (path.length > 0) {

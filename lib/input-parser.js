...
class InputParser {
    ...
    parse(options) {
        ...
        let allNodeInput = clone(newOptions.nodeInput);
        ...
        allNodeInput.forEach((ni, ind) => {
            ...
            if (Array.isArray(currNi)) {
                currNi.forEach((value, key) => {
                    isThereBatch |= this._handleNodeInput(ind, { [key]: value }, newOptions, storage, input);
                });
            }
            else {
                isThereBatch |= this._handleNodeInput(ind, currNi, newOptions, storage, input);
            }
        });
    ...
    _handleNodeInput(ind, niValue, newOptions, storage, input) {
        ...
        if (isBatch) {
            ...
            input.push(newOptions.nodeInput[ind]);
            return false;
        }
    }

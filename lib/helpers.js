...
class Helpers {
    ...
    isNode(input) {
        ...
        if (this.isReference(input)) {
            ...
            result.type = this.inputToRelation(input);
            result.path = path;
        }
        return result;
    }
    ...
    inputToRelation(input) {
        const entry = Object.entries(INPUTS_TO_RELATIONS).find(([k]) => this._startsWith(input, k));
        const object = entry ? entry[1] : undefined;
        return object;
    }
